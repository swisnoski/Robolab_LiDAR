...
class serial_URG(serial.Serial):
  ...
  def get_scantime_table(self):
    ...
    time_1scan = 60. / scsp
    self.scantime_table = np.linspace(0, time_1scan, int(ares)+1)[int(self.start_step): int(self.end_step)+1]
    return self.scantime_table

  
  def get_scanangle_table(self):
    ares = float(self.parameter["ARES"])
    angle_list = np.linspace(0, 2*np.pi, int(ares)+1)
  ...
  def capture_GDGS(self, byte=3):
    ...
    dist = np.array(decode(data, byte), float)
...
def decode(encode_str, byte = None):
  ...
  dec = dec.reshape(int(_len/byte), int(byte)) * shift6bit
